...
class AndTest(unittest.TestCase):
  def identity_test1(self):
    ...
    for val in ['a', 'b', 'c', Expression.kLastVal]:
      self.assertEqual(node.step(), val)
      self.assertEqual(node.currentValue, val)
  ...
  def test_encode(self):
    ...
    while node1.currentValue != Expression.kLastVal:
      self.assertEqual(node1.step(), node2.step())
      self.assertEqual(node1.currentValue, node2.currentValue)

class AndWithNegationsText(unittest.TestCase):
  def test1(self):
    node = And(
      ListINode(['a', 'b', 'c', 'd', 'e']),
      ListINode(['a', 'c', 'd']),
